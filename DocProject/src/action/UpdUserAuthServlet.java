...
public class UpdUserAuthServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            sysUser.setAUTH(request.getParameter("AUTH") != null ? request
                    .getParameter("AUTH").trim() : "");

            sysUser.setUserName(session.getAttribute("user_No") != null ? ((String) session
                    .getAttribute("user_No")).trim() : "");

            ArrayList<String> docIDString = new ArrayList<String>();
            if (!sysUser.getAUTH().equals(Const.Manager)
                    && request.getParameterValues("ids[]") != null) {
                // 防止一開始狀態是管理者然後亂點文件
            ...
            for (int i = 0; i < sysUser.getDocIDs().size(); i++) {
                docAuth.add(request.getParameter("DocAuth"
                        + sysUser.getDocIDs().get(i)) != null ? request
                        .getParameter("DocAuth" + sysUser.getDocIDs().get(i))
                        .trim() : "");
            }
            sysUser.setDocAuth(docAuth);

            if (sysUser.getUid().equals("") || sysUser.getAUTH().equals("")
