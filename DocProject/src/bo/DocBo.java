...
import dao.DocCtlDao;
import dao.DocDtlDao;
import dao.DocTypeDao;
import dao.SysUserDao;
import dao.UserAuthDao;
import entity.DocCtl;
import entity.DocDtl;
import entity.DocIDsAndAuth;
...
public class DocBo {
	...
	public DocBo() throws Exception {
		// conn =
		// new DBUtil().getSQLConn(Const.dB_ServerIP, Const.dB_ServerPort,
		// Const.dB_DatabaseName, Const.dB_ID,
		// Const.dB_PassWD_IV);
	}
	...
	public void updUserAuthForReadDoc(String[] modifyList, String docID, String user_No) throws SQLException {
		if (dao_UserAuth == null) {
			dao_UserAuth = new UserAuthDao(conn);
		}
		try {
	...
	public void updUserAuthForReadDoc(String[] modifyList, ArrayList<String> auth, String docID, String user_No)
			throws SQLException {
		if (dao_UserAuth == null) {
			dao_UserAuth = new UserAuthDao(conn);
		}
	...
	public ArrayList<String> getUserAuth_NotInDoc(String docID) throws SQLException {
		if (dao_UserAuth == null) {
			dao_UserAuth = new UserAuthDao(conn);
		}
	...
	public ArrayList<DocCtl> getDocCtlandDocDtl(String key) throws SQLException {
		...
		try {
			...
			user_array_all_fila.get(0).setDocIDsAndAuth(dao_UserAuth.getUserAuthForDocID(key));
			this.conn.commit();
		} catch (Exception e) {
			try {
				this.conn.rollback();
	...
	public void deleteDocDtl(String docID, String docSN, String userID) throws SQLException {
		if (dao_DocDtl == null) {
			dao_DocDtl = new DocDtlDao(conn);
		}
		try {
			...
			dao_DocDtl.addDocDtlHis(docID, userID, Const.getKey(Const.DocID_Type_Dtl), docSN);
	...
	public ArrayList<DocCtl> getDocType(String key, String uid) throws SQLException {
		if (dao_DocCtl == null) {
			dao_DocCtl = new DocCtlDao(conn);
		}
	...
	// 改這邊 並且依樣 建立兩個conn
	public void deleteDocCtl(String docID, String userID, String batchNum) throws SQLException {
		if (dao_DocDtl == null) {
			dao_DocDtl = new DocDtlDao(conn);
		}
		if (dao_DocCtl == null) {
