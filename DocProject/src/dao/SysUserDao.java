...
public class SysUserDao {
    ...
    private ArrayList<SysUser> getSysUsers(PreparedStatement pstmt)
            throws SQLException {
    ...
    public void uptSysUser(SysUser sysUser) throws SQLException {
        ...
        String sql = "Update  SysUser"
                + "  Set   isAction = ? , Sysid = ? , Sysdt = ?  where  [Uid] = ?  ";
        PreparedStatement pstmt = conn.prepareStatement(sql);
    ...
    public void addSysUser(SysUser sysUser) throws SQLException {
        // 增加使用者
        String sql = "insert into SysUser  ( [Uid], [AUTH] , [isAction] , Sysid  ,Sysdt )"
                + " values (?,?,?,?,?)";
        PreparedStatement pstmt = conn.prepareStatement(sql);
