...
public class UserAuthDao {
	...
	public void addUserAuth(String[] modifyList, ArrayList<String> auth, String docID, String user_No)
			throws SQLException {
		String sql = " insert into UserAuth ([Uid],DocID,Sysid ,Sysdt,DocAuth )  Values (?,?,?,?,?)";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		for (int i = 0; i < modifyList.length; i++) {
	...
	private ArrayList<UserAuth> getUserAuths(PreparedStatement pstmt) throws SQLException {
	...
	public ArrayList<UserAuth> getUserAuth(String docID) throws SQLException {
		String sql = "select * " + " from EsunDoc.dbo.UserAuth " + " where DocID = ? ";
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<String> getUserAuth_NotInDoc(String docID) throws SQLException {
		String sql = "select  [Uid]  from SysUser" + " where AUTH = 'U' and isAction = 'Y' "
	...
	public SysUser getUserId(String uid) throws SQLException {
		...
		// 並且傳回 可以觀看的文章數
		String sql = "Select * from SysUser  Left join UserAuth on  " + "[SysUser].[Uid] = UserAuth.[Uid] "
				+ " Left join DocCtl on " + " UserAuth.DocID = DocCtl.DocID " + " Where [SysUser].[Uid] = ?"
